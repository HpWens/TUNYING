...
public class XhsEmoticonsKeyBoard extends AutoHeightLayout implements View.OnClickListener, EmoticonsFuncView.OnEmoticonsPageViewListener,
    ...
    protected void initView() {
        mKeyboardView = findViewById(R.id.input_layout);
        ...
        mEtChat.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                if (!TextUtils.isEmpty(s)) {
                    ...
                    mBtnSend.setTextColor(Color.parseColor("#AB8349"));
                    ...
                    mBtnSend.setTextColor(Color.parseColor("#6F000000"));
    ...
    public void reset() {
        ...
        mBtnFace.setImageResource(R.mipmap.mei_input_emoji);
    }
    ...
    public void onFuncChange(int key) {
        if (FUNC_TYPE_EMOTION == key) {
            // mBtnFace.setImageResource(R.mipmap.mei_input_emoji);
            mBtnFace.setImageResource(R.mipmap.mei_input_keyboard);
        } else {
            // mBtnFace.setImageResource(R.mipmap.mei_input_emoji);
            mBtnFace.setImageResource(R.mipmap.mei_input_emoji);
        }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {
            int x = (int) ev.getX();
            int y = (int) ev.getY();
            if (!isTouchEmotionRegion(x, y) && mLyKvml.isShown()) {
                reset();
                return true;
            }
        }
     ...
     * @return
     */
    public boolean isTouchEmotionRegion(int x, int y) {
        boolean isContained = false;
        if (mKeyboardView != null) {
            Rect rect = new Rect();
            mKeyboardView.getGlobalVisibleRect(rect);
            isContained = rect.contains(x, y);
        }
        if (!isContained) {
            Rect rect = new Rect();
            mLyKvml.getGlobalVisibleRect(rect);
            isContained = rect.contains(x, y);
        }
        return isContained;
    }
